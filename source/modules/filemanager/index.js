...
class FileManager {
  constructor(opts) {
    ...
    this.config = JSON.parse(
      antSword["storage"]("adefault_filemanager", false, JSON.stringify(config))
    );
    this.isWin = true;
    this.path = "/";
    this.home = "/";
  ...
  initUI(ret) {
    ...
    if (info_path.substr(0, 1) === "/") {
      ...
      // windows 盘符统一大写
      info_path = `${info_path.substr(0, 1).toUpperCase()}${info_path.substr(
        1
      )}`;
  ...
  getFiles(p, callback) {
    ...
    if (self.isWin) {
      ...
      p =
        p.substr(1, 2) == ":/"
          ? `${p.substr(0, 1).toUpperCase()}${p.substr(1)}`
          : p;
    }
    ...
    if (self.isWin) {
      ...
      path =
        path.substr(1, 2) == ":/"
          ? `${path.substr(0, 1).toUpperCase()}${path.substr(1)}`
          : path;
    }
    let cache;
    ...
    this.path = path;
    let cache_tag =
      "filemanager-files-" + Buffer.from(this.path).toString("base64");
    if (
      (this.opts.otherConf || {})["filemanager-cache"] !== 0 &&
      (cache = this.cache.get(cache_tag))
    ) {
    ...
    this.core
      ...
      .catch((err) => {
        toastr.error(
          err instanceof Object ? JSON.stringify(err) : String(err),
          LANG_T["error"]
        );
          ...
          ((p) => {
            ...
            this.core
              ...
              .catch((err) => {
                this.files.cell.progressOff();
                // 请求失败的逻辑
                const logMessage = `Failed to delete file: ${path}, Error: ${err}`;
    ...
    layer.prompt(
      {
        ...
        title: `<i class="fa fa-fa fa-font"></i> ${
          LANG["rename"]["title"]
        } (${antSword.noxss(name)})`,
      },
      (value, index, elem) => {
        ...
        this.core
          .request(
            this.core.filemanager.rename({
              path: this.path + name,
              name:
                this.path + value + (isDir && !value.endsWith("/") ? "/" : ""),
            })
          )
          .then((res) => {
            let ret = res["text"];
    ...
    layer.prompt(
      ...
      (value, i, e) => {
        ...
        this.core
          ...
          .catch((err) => {
            toastr.error(
              LANG["createFolder"]["error"](value, err),
              LANG_T["error"]
            );
          });
        layer.close(i);
    ...
    layer.prompt(
      ...
      (value, i, e) => {
        ...
        this.core
          ...
          .catch((err) => {
            toastr.error(
              LANG["createFile"]["error"](value, err),
              LANG_T["error"]
            );
          });
        layer.close(i);
  ...
  chmodFile(name, oldmod) {
    layer.prompt(
      ...
      (value, i, e) => {
        if (
          this.core.constructor.name == "PSWINDOWS" &&
          !value.match(/^(-|d)(-|a)(-|r)(-|h)(-|s)$/)
        ) {
  ...
  previewFile(name, size) {
    ...
    this.core
      .download(
        ...
        (_size) => {
          down_size += _size;
          let down_progress = parseInt(
            parseFloat(down_size / size).toFixed(2) * 100
          );
          if (!(down_progress % 5)) {
            win.setText(
              `Preview File: ${antSword.noxss(remote_path)} ${down_progress}%`
            );
          }
        }
      )
          ...
          switch (filemime) {
            default:
              ...
              win.attachHTMLString(
                `<img style="width:100%" src="data:/${filemime};base64,${data}"/>`
              );
              break;
          }
          fs.unlink(savepath);
  ...
  downloadFile(name, size) {
    ...
    const task = this.tasks.new(
      LANG["download"]["task"]["name"],
      path,
      LANG["download"]["task"]["wait"]
    );
    // 获取要保存的路径
  ...
  downloadFileMulti(names = []) {
    ...
    names.forEach((item) => {
      ...
      self.core
        .download(
          filePath,
          self.core.filemanager.download_file({
            path: path,
          }),
          (_size) => {
            // 计算进度百分比
            down_size += _size;
            let down_progress = parseInt(
              parseFloat(down_size / item.size).toFixed(2) * 100
            );
            if (!(down_progress % 5)) {
              task.update(down_progress + "%");
            }
  ...
  wgetFile() {
    ...
    let _index = layer.prompt({
      ...
      content:
        '<input type="text" style="width:300px;" class="layui-layer-input" id="url_' +
        hash +
        '" value="http://" placeholder="target url"><p/><input style="width:300px;" type="text" id="path_' +
        hash +
    ...
    new Promise((res, rej) => {
      ...
      .then((filePaths) => {
        ...
        filePaths.map((f) => {
          ...
          tasks[f] = this.tasks.new(
            LANG["upload"]["task"]["name"],
            `${fileName} => ${path}`,
        ...
        const upload = () => {
          new Promise((res, rej) => {
            ...
            if (parseInt((this.opts.otherConf || {})["upload-fragment"]) > 0) {
              dataSplit =
                parseInt((this.opts.otherConf || {})["upload-fragment"]) * 1024;
            }
            ...
            const uploadBuffFunc = (_buff) => {
              new Promise((res, rej) => {
                ...
                task.update(
                  `${parseInt(
                    ((buffLength - b.length * _buff.length) / buffLength) * 100
                  )}%`
                );
                this.core
                  .request(
                  ...
                  .catch((err) => {
                    ...
                    toastr.error(
                      LANG["upload"]["error"](fileName, errmsg),
                      LANG_T["error"]
                    );
                  });
              });
    ...
    toolbar.attachEvent("onClick", (id) => {
      if (id === "save") {
        ...
        self.core
          ...
          .then((res) => {
            ...
            if (ret === "1") {
              ...
              toastr.error(
                LANG["editor"]["error"](path, ret === "0" ? "" : "<br/>" + ret),
                LANG_T["error"]
              );
            }
          })
        ...
        editor.session.setValue(
          iconv.decode(Buffer.from(codes), encode).toString()
        );
      } else if (id === "refresh") {
        // 获取文件代码
        win.progressOn();
    ...
    self.core
      ...
      .then((ret) => {
        ...
        for (let i = 0; i < _data.length; i++) {
          ...
          data_arr.push({
            id: i + 1,
            data: [antSword.noxss(item[0]), antSword.noxss(item[1])],
          });
        }
        grid.parse(
          {
